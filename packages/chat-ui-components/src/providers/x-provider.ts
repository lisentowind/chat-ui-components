 ...
 * 提供主题、国际化、全局配置等功能
 */
export class XProvider extends LitElement {
  static styles = css`
    :host {
      display: none;
    }
  ...
  // 全局单例
  private static _instance: XProvider | null = null;

  static get instance(): XProvider | null {
...
// 自定义元素注册
if (!customElements.get("x-provider")) {
  customElements.define("x-provider", XProvider);
}

// 便捷函数
export function getXProvider(): XProvider | null {
  return XProvider.instance;
}
