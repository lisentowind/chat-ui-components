...
import { getXProvider } from "../providers/x-provider";

/**
 * XNotification - 系统通知组件
 * 提供各种类型的系统通知显示
 */
@customElement("x-notification")
export class XNotification extends LitElement {
  static styles = css`
    :host {
      position: fixed;
      z-index: 1000;
    ...
    .notification {
      background: var(--x-notification-success-bg, #f0f9ff);
      color: var(--x-notification-success-text, #0c4a6e);
      border: 1px solid var(--x-notification-success-border, #7dd3fc);
      border-radius: 8px;
      padding: 12px 16px;
      margin-bottom: 8px;
    ...
    .notification.error {
      background: var(--x-notification-error-bg, #fef2f2);
      color: var(--x-notification-error-text, #991b1b);
      border-color: var(--x-notification-error-border, #fca5a5);
    }

    .notification.warning {
      background: var(--x-notification-warning-bg, #fffbeb);
      color: var(--x-notification-warning-text, #92400e);
      border-color: var(--x-notification-warning-border, #fcd34d);
    }

    .notification.info {
      background: var(--x-notification-info-bg, #f0f9ff);
      color: var(--x-notification-info-text, #075985);
      border-color: var(--x-notification-info-border, #7dd3fc);
    }
...
export class NotificationManager {
  ...
  private _notifications: Array<{ id: string; element: XNotification }> = [];
  private _maxCount = 5;
